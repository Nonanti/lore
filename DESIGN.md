...
> **Status (629 tests passing + 3 ignored [live-LLM/neural], clippy clean, with CI):** M0–M31 ✅ + AI coworkers ✅ + code review fixes ✅ + 4-way review hardening ✅ + Phase E hardening ✅ + Phase E review hardening ✅ + native tool calling ✅ incl. Codex (`docs/superpowers/specs/2026-07-24-native-tool-calling-design.md`).
...
| D19 | Native tool calling: structured `Thread`/`ContentBlock` model at the core, `Model::complete_thread` beside `complete`; `solve` dispatches text/native drivers over one shared epilogue; `tool_mode: auto\|native\|text` per `ModelConfig` with a per-agent downgrade latch on typed unsupported errors. All three real providers speak it (Anthropic blocks, OpenAI `tool_calls`, Codex Responses items); Mock stays text (test double) | Providers train models on the `tool_use`/`tool_result` protocol — text-parsed JSON was the source of two production bugs. Text stays as the fallback for tool-incapable endpoints (gemma, R1 distills, misconfigured vLLM); flat text becomes a rendering, not the foundation |
| D20 | Streaming tool use: **deliberately deferred** — `complete_thread` is one-shot; SSE tool-use assembly (`input_json_delta`, `tool_calls` deltas) is not built | No consumer exists: chat streaming surfaces carry no tools, and `solve` is batch (results land in task logs). Building the assembly now is dead code with real maintenance cost; the block model makes it a bounded add the day a streaming-solve UX exists |
| D22 | Team memory: `Scope::World` IS the team scope (no new enum variant/migration); `CompositeStore` routes writes by scope (World → shared `memory/team.db`, Agent → personal file) and merges recalls; distillation shares Convention + Constraint items only (facts/procedures stay personal — Wilson stats must not mix); `--no-share` opts out per agent; cross-agent dedup is consolidation's job | Agents on a team must learn from each other's tasks without a human relay; the storage-level composite keeps every layer above the trait store-agnostic, and "visible to all agents" already had a name — World |
| D23 | Dashboard is a **single embedded HTML file** (`GET /ui`, `include_str!`, inline CSS/JS, system fonts, zero CDN/npm/build step) in the public route group; every data call is client-side with the user's Bearer key; polling over streams (U4/D20 discipline); no new server endpoints — pure consumer of the existing API | "Everything lives inside the binary" applies to the UI too; a node toolchain would be the project's first external build dependency. The static shell is secret-free, so the auth wall stays exactly where it was |
| D21 | Memory deepening is **eval-first, offline-first**: hardened golden set (56 records / 32 categorized queries, hit@1/5 + MRR floors) gates every retrieval change; graph leg (incremental entity index, damped 1-hop, acronym entities) ships in the default build; neural stack (e5 + cross-encoder `NeuralReranker`, `LORE_RERANKER=neural`) stays opt-in. Measured: 72% → 78% offline → 100% neural hit@5. ColBERT rejected; native-rerank tuning and RAPTOR-lite deferred **with eval evidence** (spec §6) | "Memory quality is the product" needs numbers, not vibes; the saturated 15/15 benchmark was replaced before any signal work. Offline default keeps the self-contained identity; every deferral carries its measurement so revisiting is mechanical |
...
> **Status (629 tests passing + 3 ignored [live-LLM/neural], clippy clean, with CI):** M0–M31 ✅ + AI coworkers ✅ + code review fixes ✅ + 4-way review hardening ✅ + Phase E hardening ✅ + Phase E review hardening ✅ + native tool calling ✅ incl. Codex + memory deepening ✅ (D21) + team memory ✅ (D22) + dashboard ✅ (D23). **Roadmap docs:** [`2026-07-24-next-roadmap.md`](../docs/superpowers/specs/2026-07-24-next-roadmap.md) (phases A–E, done) · [`2026-07-24-native-tool-calling-design.md`](../docs/superpowers/specs/2026-07-24-native-tool-calling-design.md) (done incl. the Codex addendum; streaming tool use deliberately deferred — D20).
